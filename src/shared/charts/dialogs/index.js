...
import { Dialog, DialogContent, Typography, IconButton, Tab, Tabs, Box } from '@material-ui/core';
...
class Dialogs extends Component {
	...
	fetchTimeSeriesData = async (from, to, limit) => {
		...
		if (this.props.dataPoint === 'mindsphere') {
			...
			const response = await loadDataByGivenDate(
				from,
				to,
				this.props.assetId,
				this.props.aspectName,
	...
	fetchAgreegateData = async (from, to, limit, intervalUnit, intervalValue) => {
		if (this.props.dataPoint === 'mindsphere') {
			const fromDate = zonedTimeToUtc(from, Intl.DateTimeFormat().resolvedOptions().timeZone);
			const toDate = zonedTimeToUtc(to, Intl.DateTimeFormat().resolvedOptions().timeZone);
			const response = await loadAggregateDataV2(
				from,
				to,
				this.props.assetId,
				this.props.aspectName,
			...
			>
				...
				<DialogContent>
					<div className={this.props.classes.root}>
						...
						<div role='tabpanel' id='tab-0'>
							<Typography variant='h6' style={{ paddingLeft: 10 }}>
								{this.state.tabIndex === 0 ? 'Seven Day' : this.state.tabIndex === 1 ? 'One Month' : 'Three Month'}
							</Typography>

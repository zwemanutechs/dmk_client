...
import { BASEURI, API, IS_DEV } from '../constants/api-constants';

export const client = axios.create({
	baseURL: IS_DEV ? API + BASEURI : BASEURI,
	// headers: {
...
export const mindsphere = axios.create({
	...
	headers: {
		...
		'Content-Security-Policy':
			"default-src 'self'; script-src 'self' https://dokaiot-fleetmanager.eu1.mindsphere.io 'self'",
	},
});
	...
	(error) => {
		if (IS_DEV) {
			store.dispatch({
				type: OPEN_SNACK,
				status: true,
				message: error && error.response ? error.response.data.message : UNHANDELERROR,

...
import ConversionAddOrEdit from "./conversion-addOrEdit";
...
import {conversionModel} from "../model/model";
...
class Conversion extends Component {
    ...
    getData = async (pageNo) => {
        const response = await get(`conversion?pageNo=${pageNo === undefined ? 0: pageNo}&pageSize=${10}`);
        if(response){
            this.setState(state =>
                ({
    ...
    handelFormSubmit = async () => {
        ...
        if(hasError && !hasError.valid){
            ...
            if(this.props.title === 'UPDATE'){
                const response = await put('conversion/update', this.state.formData);
                if(response && response.data.code){
                    const dataIndex = this.state.tableData.findIndex(x => x.id === response.data.data.id);
                    if(dataIndex >= 0){
                        const tableData = this.state.tableData;
                        tableData.splice(dataIndex, 1);
                        this.setState(state => ({tableData: [...tableData, response.data.data], formData: conversionModel, onProgress: false}), () => this.props.closeDialog(false, ''));
                    }
                }
            }else{
                const response = await post('conversion/add', this.state.formData);
                if(response && response.data.code){
                    this.setState(state => ({tableData: [response.data.data, ...state.tableData], formData: conversionModel, onProgress: false,totalCount:state.totalCount+1}), () => this.props.closeDialog(false, ''));
                }
            }
        }
    };
    ...
    onDelete = (rowData) => new Promise(async (resolve, reject) => {
        if(Array.isArray(rowData.data) && rowData.data.length > 0){
                ...
                const response = await deleteRange('conversion/deleterange', deleteList);
                if(response && response.data.code){
    ...
    onFormClose = async () => {
        ...
        this.setState(state => ({formData: conversionModel}));
    };
        ...
        return (
            <Grid container direction="row" justify="center">
                <Grid item xs={12}>
                     ...
                     * **/}
                    <MaxWidthDialog
                        content={<ConversionAddOrEdit dataSet={this.state.formData} handelChange={this.handelChange}  onFormSubmit={this.state.onProgress} formError={this.state.formError}/>}

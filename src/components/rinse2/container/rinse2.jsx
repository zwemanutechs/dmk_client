...
class Rinse2 extends Component {
    ...
    getData = async (pageNo) => {
        const response = await get(`rinse2?pageNo=${pageNo === undefined ? 0: pageNo}&pageSize=${10}`);
        if(response){
            this.setState(state =>
                ({
            ...
            if(this.props.title === 'UPDATE'){
                ...
                if(response && response.data.code){
                    ...
                    if(dataIndex >= 0){
                        ...
                        this.setState(state => ({tableData: [...tableData, response.data.data], formData: r2Model, onProgress: false,}), () => this.props.closeDialog(false, ''));
                    }
                }
            }else{
                const response = await post('rinse2/add', this.state.formData);
                if(response && response.data.code){
                    this.setState(state => ({tableData: [response.data.data, ...state.tableData], formData: r2Model, onProgress: false,totalCount:state.totalCount+1}), () => this.props.closeDialog(false, ''));
                }
            }
        }
    };
        ...
        return (
            <Grid container direction="row" justify="center">
                <Grid item xs={12}>
                     ...
                     * **/}
                    <MaxWidthDialog
                        content={<RinseTwoAddOrEdit dataSet={this.state.formData} handelChange={this.handelChange}  onFormSubmit={this.state.onProgress} formError={this.state.formError}/>}
                        contentTitle={"RINSE TWO"}

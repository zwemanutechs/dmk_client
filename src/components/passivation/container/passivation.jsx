...
import PassivationAddOrEdit from "./passivation-addOrEdit";
...
import {passivationModel} from "../model/model";
...
const columns = [
    ...
    {label: 'Concentration Below 0.5% Top-up Chemical', name: 'concentrationBelowTopUp',
        options: {
            filter: false,
...
class Passivation extends Component {
    ...
    getData = async (pageNo) => {
        const response = await get(`passivation?pageNo=${pageNo === undefined ? 0: pageNo}&pageSize=${10}`);
        if(response){
            this.setState(state =>
                ({
    ...
    handelFormSubmit = async () => {
        ...
        if(hasError && !hasError.valid){
            ...
            if(this.props.title === 'UPDATE'){
                const response = await put('passivation/update', this.state.formData);
                if(response && response.data.code){
                    const dataIndex = this.state.tableData.findIndex(x => x.id === response.data.data.id);
                    if(dataIndex >= 0){
                        const tableData = this.state.tableData;
                        tableData.splice(dataIndex, 1);
                        this.setState(state => ({tableData: [...tableData, response.data.data], formData: passivationModel, onProgress: false}), () => this.props.closeDialog(false, ''));
                    }
                }
            }else{
                const response = await post('passivation/add', this.state.formData);
                if(response && response.data.code){
                    this.setState(state => ({tableData: [response.data.data, ...state.tableData], formData: passivationModel, onProgress: false,totalCount:state.totalCount+1}), () => this.props.closeDialog(false, ''));
                }
            }
        }
    };
    ...
    onDelete = (rowData) => new Promise(async (resolve, reject) => {
        if(Array.isArray(rowData.data) && rowData.data.length > 0){
                ...
                const response = await deleteRange('passivation/deleterange', deleteList);
                if(response && response.data.code){
    ...
    onFormClose = async () => {
        ...
        this.setState(state => ({formData: passivationModel}));
    };
        ...
        return (
            <Grid container direction="row" justify="center">
                <Grid item xs={12}>
                     ...
                     * **/}
                    <MaxWidthDialog
                        content={<PassivationAddOrEdit dataSet={this.state.formData} handelChange={this.handelChange}  onFormSubmit={this.state.onProgress} formError={this.state.formError}/>}
                        contentTitle={"PASSIVATION"}

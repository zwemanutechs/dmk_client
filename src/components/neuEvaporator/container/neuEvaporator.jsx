...
const columns = [
    ...
    {
        label: 'Water Sample In Bottle Tank6', name: 'waterSampleInBottleTank6', options: {
            filter: false,
    ...
    {
        label: 'Monthly Calibration Of Ph Meter Tank3', name: 'monthlyCalibrationOfPhMeterTank3', options: {
            filter: false,
            customBodyRender: (value, tableMeta, updateValue) => (
                <span>
              {value ? "Done" : "Not Done"}
...
class NeuEvaporator extends Component {
    ...
    getData = async (pageNo) => {
        const response = await get(`neuEvaporator?pageNo=${pageNo === undefined ? 0 : pageNo}&pageSize=${10}`);
        if (response) {
            this.setState(state =>
                ({
    ...
    onDelete = (rowData) => new Promise(async (resolve, reject) => {
        if (Array.isArray(rowData.data) && rowData.data.length > 0) {
            ...
            const response = await deleteRange('neuEvaporator/deleterange', deleteList);
            if (response && response.data.code) {
        ...
        return (
            <Grid container direction="row" justify="center">
                <Grid item xs={12}>
                     ...
                     * **/}
                    <MaxWidthDialog
                        content={<NeuEvaporatorAddOrEdit dataSet={this.state.formData} handelChange={this.handelChange}

...
class RinseDi extends Component {
    ...
    getData = async (pageNo) => {
        const response = await get(`rinsedi?pageNo=${pageNo === undefined ? 0: pageNo}&pageSize=${10}`);
        if(response){
            this.setState(state =>
                ({
            ...
            if(this.props.title === 'UPDATE'){
                ...
                if(response && response.data.code){
                    ...
                    if(dataIndex >= 0){
                        ...
                        this.setState(state => ({tableData: [...tableData, response.data.data], formData: rdiModel, onProgress: false,}), () => this.props.closeDialog(false, ''));
                    }
                }
            }else{
                const response = await post('rinsedi/add', this.state.formData);
                if(response && response.data.code){
                    this.setState(state => ({tableData: [response.data.data, ...state.tableData], formData: rdiModel, onProgress: false,totalCount:state.totalCount+1}), () => this.props.closeDialog(false, ''));
                }
            }
        }
    };

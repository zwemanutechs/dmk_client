...
import DegreasingAddOrEdit from "./degreasing-addOrEdit";
...
const columns = [
    ...
    {label: 'Concentration Below 0.5% Top-up Chemical', name: 'concentrationTopUp',
        options: {
            filter: false,
...
class Degreasing extends Component {
    ...
    getData = async (pageNo) => {
        const response = await get(`degreasing?pageNo=${pageNo === undefined ? 0: pageNo}&pageSize=${10}`);
        if(response){
            this.setState(state =>
                ({
    ...
    handelFormSubmit = async () => {
        ...
        if(hasError && !hasError.valid){
            ...
            if(this.props.title === 'UPDATE'){
                const response = await put('degreasing/update', this.state.formData);
                if(response && response.data.code){
                    const dataIndex = this.state.tableData.findIndex(x => x.id === response.data.data.id);
                    if(dataIndex >= 0){
                        const tableData = this.state.tableData;
                        tableData.splice(dataIndex, 1);
                        this.setState(state => ({tableData: [...tableData, response.data.data], formData: degreasingModel, onProgress: false}), () => this.props.closeDialog(false, ''));
                    }
                }
            }else{
                const response = await post('degreasing/add', this.state.formData);
                if(response && response.data.code){
                    this.setState(state => ({tableData: [response.data.data, ...state.tableData], formData: degreasingModel, onProgress: false,totalCount:state.totalCount+1}), () => this.props.closeDialog(false, ''));
                }
            }
        }
    };
    ...
    onDelete = (rowData) => new Promise(async (resolve, reject) => {
        if(Array.isArray(rowData.data) && rowData.data.length > 0){
                ...
                const response = await deleteRange('degreasing/deleterange', deleteList);
                if(response && response.data.code){
    ...
    onFormClose = async () => {
        ...
        this.setState(state => ({formData: degreasingModel}));
    };
        ...
        return (
            <Grid container direction="row" justify="center">
                <Grid item xs={12}>
                     ...
                     * **/}
                    <MaxWidthDialog
                        content={<DegreasingAddOrEdit dataSet={this.state.formData} handelChange={this.handelChange}  onFormSubmit={this.state.onProgress} formError={this.state.formError}/>}
                        contentTitle={"DEGREASING"}

...
class PaintBooth extends Component {
    ...
    getData = async (pageNo) => {
        const response = await get(`paintBooth?pageNo=${pageNo === undefined ? 0 : pageNo}&pageSize=${10}`);
        if (response) {
            this.setState(state =>
                ({
    ...
    onDelete = (rowData) => new Promise(async (resolve, reject) => {
        if (Array.isArray(rowData.data) && rowData.data.length > 0) {
            ...
            const response = await deleteRange('paintBooth/deleterange', deleteList);
            if (response && response.data.code) {
        ...
        return (
            <Grid container direction="row" justify="center">
                <Grid item xs={12}>
                     ...
                     * **/}
                    <MaxWidthDialog
                        content={<PaintBoothAddOrEdit dataSet={this.state.formData} handelChange={this.handelChange}

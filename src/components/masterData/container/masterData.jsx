...
import MasterDataAddOrEdit from "./masterData-addOrEdit";
import {
  openDialog,
  closeDialog,
...
import { masterDataModel } from "../model/model";
import {
  get,
  post,
  put,
...
class MasterData extends Component {
  ...
  getData = async (pageNo) => {
    const response = await get(
      `masterdata?pageNo=${pageNo === undefined ? 0 : pageNo}&pageSize=${10}`
    );
    if (response) {
      this.setState((state) => ({
  ...
  handelFormSubmit = async () => {
    ...
    if (hasError && !hasError.valid) {
      ...
      if (this.props.title === "UPDATE") {
        const response = await put("masterdata/update", this.state.formData);
        ...
        const response = await post("masterdata/add", this.state.formData);
        if (response && response.data.code) {
          this.setState(
            (state) => ({
  ...
  onFormClose = async () => {
    ...
    this.setState((state) => ({ formData: masterDataModel }));
  };
